...
`bot.strategies.max_market_trades_per_product` is an optional positive integer that caps accepted market trades retained per product when the strategy harness replays a snapshot. Use it to bound strategy snapshot memory on long-running ledgers. Snapshot projection payloads include `market_trade_retention` with the configured cap and dropped-trade counts. Strategies that require complete historical windows should leave this unset or explicitly reject snapshots where retention metadata reports dropped trades.
