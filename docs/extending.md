...
Venue and product capability helpers report the current StateRail adapter contract, not every feature a venue may expose. Coinbase spot (`CBE`) and Coinbase Financial Markets futures (`FCM`) are live-executable through the current adapter. Coinbase INTX metadata can be replayed, but live INTX order routing is disabled. Amend, reduce-only forwarding, and attached orders are reported as unsupported until this adapter implements them.
